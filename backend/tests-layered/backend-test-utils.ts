...
export const createTestApp = async (options: CreateTestAppOptions = {}): Promise<TestAppContext> => {
  ...
  jest.doMock("../src/core/cache/redis", () => ({
  ...
  jest.doMock("../src/services/llm/incident-analyzer", () => ({
  ...
  jest.isolateModules(() => {
    ...
    const { errorHandler, notFoundHandler } = require("../src/core/http/error-handler");

    app = express();
    app.use(express.json());

    if (options.surface === "workflow") {
      const { approvalsRouter } = require("../src/api/routes/approvals.routes");
      const { actionsRouter } = require("../src/api/routes/actions.routes");

      app.use("/api/approvals", approvalsRouter);
      app.use("/api", actionsRouter);
    } else {
      const { llmReportsRouter } = require("../src/api/routes/llm-reports.routes");
