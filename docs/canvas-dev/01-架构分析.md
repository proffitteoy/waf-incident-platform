...
## 当前项目固定参数（waf-incident-platform）

- 项目路径：`c:/Users/84025/Desktop/waf-incident-platform`
- 重点层级：`backend/src/api`、`backend/src/services`、`backend/src/core`
- 必须识别节点：`services/llm`、`services/policy`、`core/cache/redis`
- 必须识别数据流：`analyze-events -> incident/alert/llm_report` 与 `actions -> redis action state`
- 白板输出要求：所有节点说明和连线标签使用中文
