...
- 技术栈：`Node.js + TypeScript + Express + PostgreSQL + Redis + Vue 3 + Vite`
- 本轮约束：不启用事件关联引擎，分析由 LLM API 直连完成
- 编码同步要求：
  - 若新增/修改动作执行链路，必须同步 Redis 动作状态写入与回滚清理
  - 若新增路由，必须同步更新 `docs/api-spec.md` 与 `docs/技术细节.md`
  - 所有新增目录必须创建 README（中文）
